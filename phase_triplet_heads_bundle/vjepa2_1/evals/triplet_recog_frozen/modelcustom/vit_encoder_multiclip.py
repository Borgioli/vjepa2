...
class ClipAggregation(nn.Module):
    ...
    def __init__(
        ...
        max_frames=128,
        use_pos_embed=False,
        token_layout="temporal_spatial_flatten",
    ):
        super().__init__()
        self.model = model
        ...
        if self.token_layout not in (
            "temporal_spatial_flatten",
            "spatial_only_mean",
            "temporal_only_mean",
        ):
            raise ValueError(
                f"Unsupported token_layout '{self.token_layout}'. "
                "Expected one of: temporal_spatial_flatten, spatial_only_mean, temporal_only_mean"
            )
    ...
    def forward(self, x, clip_indices=None):
        ...
        def multiviews_postprocess(outputs):
            ...
            for i, outputs in enumerate(all_outputs):
                ...
                if self.token_layout == "temporal_spatial_flatten":
                    outputs = outputs.flatten(1, 2)
                elif self.token_layout == "spatial_only_mean":
                    outputs = outputs.mean(dim=1)
                elif self.token_layout == "temporal_only_mean":
